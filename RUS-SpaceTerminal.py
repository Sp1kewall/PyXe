...
try:
    ...
    while ask_true:
        clear()
        ask = input("""
О нет! Похоже вы не установили
обязательные модули для вашего Python! Мы
настоятельно рекомендуем установить каждый
модуль, или вы можете продолжить работу
на свой страх и риск. Установить модули?
[y/n] $:""")
    if ask == 'y' or ask == 'Y':
        ask_true = False
        if name == 'nt':
            ...
            os.system("pip install colorama")
        clear()
        print("Через 3 секунды Space Terminal завершит свою работу самостоятельно")
        time.sleep(3)
        sys.exit()
    elif ask == 'n' or ask == 'N':
        ask_true = False
