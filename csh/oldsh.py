...
while True:
    try:
        ...
        if os.path.isfile("homedir.txt"):
                homedir0 = open("homedir.txt")
                homedir = homedir0.read()
        else:
            ask_home = input("Введите путь домашней папки\n$:")
            lol = open("homedir.txt", 'w')
            lol.writelines(ask_home)
            lol.close()
            homedir0 = open("homedir.txt")
            homedir = homedir0.read()
            ...
            def shell(lex,arg):
                ...
                if lex == 'help' or lex == 'HELP':
                    if arg == '--say':
                        ...
                        print("LS - Команда для просмотра содержимого директории\n\n")
                    elif arg == '--wia':
            ...
            while True:
                try:
                    ...
                    break
                                ...
                                ask_home = input("Enter home folder path\n$:")
                                lol = open("homedir.txt", 'w')
                                lol.writelines(ask_home)
                                lol.close()
                                homedir0 = open("homedir.txt")
